...
- Authorization must be explicit. Every state-changing operation has a
  documented authority.
- Avoid admin powers unless absolutely necessary, and document any that exist.
- Document upgrade authority.
- Maintain strict testnet/mainnet separation.
- Include negative authorization tests: prove that the unauthorized caller is
  rejected, not only that the authorized caller succeeds.
- Test replay resistance where relevant.
- Test profile ownership transitions.
- Test username collision and normalization edge cases.
- Test wallet-linking authorization.
- Treat payment destination changes as high-risk and test them accordingly.
- Never use production secret keys in deployment scripts. Deployment scripts
  take secrets from environment variables or a secure signing mechanism.
